...
#ifdef ONVM
#include "modules/onvm/onvm_capture.h"
#endif
...
#ifdef ONVM
    struct onvm_nf_local_ctx *nf_local_ctx = onvm_capture_init(context);
#endif
...
#elif defined(ONVM)
    onvm_capture_start(context, nf_local_ctx);
    onvm_capture_stop(nf_local_ctx);
#else
