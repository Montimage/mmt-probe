...
http_reconstruct_t *http_reconstruct_alloc_init( mmt_handler_t *dpi_handler ){
	...
	static const conditional_handler_t handlers[] = {
		...
		{.proto_id = PROTO_HTTP, .att_id = HTTP_MESSAGE_START,     .handler = http_message_start_handle},
		{.proto_id = PROTO_HTTP, .att_id = HTTP_HEADER,            .handler = http_generic_header_handle},
		{.proto_id = PROTO_HTTP, .att_id = HTTP_HEADERS_END,       .handler = http_headers_end_handle},
		{.proto_id = PROTO_HTTP, .att_id = HTTP_DATA,              .handler = http_data_handle},
		{.proto_id = PROTO_HTTP, .att_id = HTTP_MESSAGE_END,       .handler = http_message_end_handle},
		{.proto_id = PROTO_HTTP, .att_id = RFC2822_RESPONSE,       .handler = NULL},
		{.proto_id = PROTO_HTTP, .att_id = RFC2822_CONTENT_TYPE,   .handler = NULL},
		{.proto_id = PROTO_HTTP, .att_id = RFC2822_URI,            .handler = NULL},
	};

	dpi_register_conditional_handler( dpi_handler,  (sizeof handlers / sizeof( conditional_handler_t)), handlers, NULL  );
	register_packet_handler( dpi_handler, 11, http_packet_handler, arg);
	register_session_timeout_handler( dpi_handler, http_classification_expiry_session, NULL);
}
