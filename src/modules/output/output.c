...
// public API
output_t *output_alloc_init(uint16_t output_id,
                            const struct output_conf_struct *config,
                            uint32_t probe_id, const char *input_src,
  ...
  // When using output in multi-threads, we need to synchronize their function
  // calls by mutex
  if (is_multi_threads) {
  ...
  ret->modules.mongodb = mongodb_output_alloc_init(
      ret->config->mongodb, ret->config->cache_max, output_id);
#endif
 ...
 * @return
 */
static inline int _write(output_t *output, output_channel_conf_t channels,
                         const char *message, bool raw) {
  int ret = 0;
...
#define __LOCK_IF_NEED(output)                                                 \
  while (output->mutex != NULL && pthread_mutex_lock(output->mutex) != 0)      \
    ;                                                                          \
/*                                                                             \
 * This macro unlocks the mutex being locked by the macro above.               \
 */
#define __UNLOCK_IF_NEED(output)                                               \
  while (output->mutex != NULL && pthread_mutex_unlock(output->mutex) != 0)    \
    ;
...
int output_write_report(output_t *output, output_channel_conf_t channels,
  ...
  if (output == NULL || output->config == NULL || !output->config->is_enable ||
...
/////////////////////// DYNABIC ////////////////////////////
#ifdef STIX_FORMAT
  message_constucted =
      construct_alert_stix_format(message_body, ts, message, sizeof(message));
  // if output->config->kafka->topic_name contains dahsboard_alerts, we force to
  // use normal format
  ...
  if (message_constucted != 1) { // Other data used the same output format
    ...
    STRING_BUILDER_WITH_SEPARATOR(offset, message,
                                  MAX_LENGTH_FULL_PATH_FILE_NAME, ",",
                                  __INT(report_type), __INT(output->probe_id),
                                  __STR(output->input_src), __TIME(ts));
...
// public API
int output_write_report_with_format(output_t *output,
                                    output_channel_conf_t channels,
  ...
  // otherwise there will be a deadlock as there will be a lock in
  // @output_write_report
  __UNLOCK_IF_NEED(output);
  ...
  if (unlikely(format == NULL)) {
    ...
    offset += vsnprintf(message + offset, MAX_LENGTH_REPORT_MESSAGE - offset,
                        format, args);
    va_end(args);
...
// public API
int output_write(output_t *output, output_channel_conf_t channels,
                 const char *message) {
  int ret;
  __LOCK_IF_NEED(output);
...
#ifdef MONGODB_MODULE
  if (output->modules.mongodb && output->config->mongodb->is_enable)
