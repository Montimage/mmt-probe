 ...
 * @param context
 */
static inline void _capture_release( probe_context_t *context ){
	int i;
	//close pcap in main thread
	fclose( context->modules.packet_captor->handler );
...
void packet_capture_start( probe_context_t *context ){
	...
	context->modules.packet_captor->handler = fd_handler;
