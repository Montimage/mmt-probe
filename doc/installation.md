...
MONGODB_MODULE           build                    
```

The options are in *UPPERCASE*, while the action are in *lowercase*. 
For example, to compile MMT-Probe to use DPDK: `make DPDK_CAPTURE build`
...
```bash
make ALL_MODULE build
```

#### Compile actions

- `build`: compile source code to obtain executable file `probe`
